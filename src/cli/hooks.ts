...
npx clarte --check || {
  ...
  git add .claude/rules/ .cursor/rules/ .cursor/agents/ .cursor/mcp.json 2>/dev/null
}
`;
...
export async function initPreCommitHook(rootDir: string): Promise<void> {
  ...
  if (await fileExists(huskyDir)) {
    ...
    console.log("");
    console.log(
      "  npx clarte --check || { npx clarte --refresh-snapshot && git add .claude/rules/ .cursor/rules/ .cursor/agents/ .cursor/mcp.json 2>/dev/null; }",
    );
    return;
  }
  ...
  for (const lf of lefthookFiles) {
    if (await fileExists(path.join(rootDir, lf))) {
      ...
      console.log(
        "        run: npx clarte --check || { npx clarte --refresh-snapshot && git add .claude/rules/ .cursor/rules/ .cursor/agents/ .cursor/mcp.json 2>/dev/null; }",
      );
      return;
    }
  }
