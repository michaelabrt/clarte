...
const ENFORCEMENT_INDEX_FILE = "enforcement-index.json";
...
// Only enforce on high-impact files (Foundation, Orchestrator, Barrel, chokepoints,
// and unclassified files above the project's p75 importer count).
// enforcement-index.json is generated by clarte and contains exactly those paths.
const indexPath = resolve(cwd, ".clarte/hooks/enforcement-index.json");
let enforced;
try { enforced = new Set(JSON.parse(readFileSync(indexPath, "utf-8"))); } catch {
  // No index = enforcement disabled gracefully
  process.exit(0);
}
const relNorm = rel.replace(/\\\\/g, "/");
if (!enforced.has(rel) && !enforced.has(relNorm)) process.exit(0);
...
/**
 * Build the enforcement index: paths of files that require clarte_context before editing.
 * Criteria (all graph-derived, no magic numbers):
 *   - role is Foundation, Orchestrator or Barrel (high blast radius by definition)
 *   - isChokepoint (structural bridge between many upstreams and downstreams)
 *   - fallback: unclassified files above the project's 75th-percentile importer count
 */
function buildEnforcementIndex(graph: PersistedGraph): string[] {
  const counts = Object.values(graph.files)
    .map((f) => f.importedByCount)
    .sort((a, b) => a - b);
  const p75 = counts[Math.floor(counts.length * 0.75)] ?? 0;

  const HIGH_IMPACT_ROLES = new Set(["Foundation", "Orchestrator", "Barrel"]);

  return Object.entries(graph.files)
    .filter(([, record]) => {
      if (record.role && HIGH_IMPACT_ROLES.has(record.role)) return true;
      if (record.isChokepoint) return true;
      if (!record.role && p75 > 0 && record.importedByCount > p75) return true;
      return false;
    })
 ...
 * When mcpEnabled=true, also generates MCP state tracking hook files.
 */
export async function generateHookFiles(
  ...
  if (mcpEnabled) {
    const enforcementIndex = buildEnforcementIndex(graph);
    await writeFileSafe(
      path.join(hooksDir, ENFORCEMENT_INDEX_FILE),
      JSON.stringify(enforcementIndex, null, 2),
    );
