...
// Generated by clarte - do not edit
// Fires on PreToolUse(Agent): writes pre-flight-done marker BEFORE the subagent
// starts, so the subagent has read access during its own tool calls.
...
try {
  ...
  writeFileSync(resolve(STATE_DIR, "pre-flight-done"), new Date().toISOString());
} catch {}
`;
