...
run_condition() {
  ...
  (cd "$work_dir" && claude -p "$ISSUE_TEXT" \
    --output-format json \
    --model "${MODEL:-sonnet}" \
...
print_results() {
  ...
  for name in placebo clarte phone-book pointer direct direct-1 direct-2 direct-3; do
...
CLARTE_CFG_EOF
  ...
  *)
    ...
    echo "Usage: TARGET=hono|directus $0 [placebo|clarte|pointer|direct|direct-1|direct-2|direct-3|parallel|r7|r9|all]"
    exit 1
    ;;
esac
